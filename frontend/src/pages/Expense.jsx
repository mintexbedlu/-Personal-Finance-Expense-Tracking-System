...
const ExpensePage = () => {
  ...
  const fetchOverview = useCallback(
    async (range = timeFrame ?? "monthly", date = selectedMonth) => {
      try {
        const res = await axios.get(`${API_BASE}/expense/overview`, {
          headers: getAuthHeaders(),
          params: { range, date: date ? date.toISOString() : undefined },
        });
    ...
    [timeFrame, selectedMonth, getAuthHeaders],
  );

  // Re-fetch overview when timeframe or selected month changes
  useEffect(() => {
    fetchOverview(timeFrame || "monthly", selectedMonth);
  }, [timeFrame, selectedMonth, fetchOverview]);
  ...
  const handleExport = async () => {
    try {
      ...
      let filename = `expense_details_${new Date().toISOString().split("T")[0]}.xlsx`;
    ...
    <div className={styles.container}>
      ...
      <div className={styles.cardsGrid}>
        ...
        <FinancialCard
          ...
          value={`ETB ${averageExpense.toLocaleString()}`}
          additionalContent={
            <div className="mt-2 text-xs text-gray-500 flex items-center">
