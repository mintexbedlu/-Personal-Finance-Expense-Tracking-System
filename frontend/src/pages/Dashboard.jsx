    ...
    <div className={dashboardStyles.container}>
      ...
      <div className={dashboardStyles.summaryGrid}>
        <FinancialCard
          ...
          label="Total Balance"
          value={`ETB ${Math.round(displayIncome - displayExpenses).toLocaleString()}`}
          additionalContent={
            <div className="flex items-center gap-2 mt-2 text-sm">
              <span className={dashboardStyles.balanceBadge}>
                Income: ETB {Math.round(displayIncome).toLocaleString()}
              </span>

              <span className={dashboardStyles.expenseBadge}>
                -ETB {Math.round(displayExpenses).toLocaleString()}
              </span>
            </div>
          }
        />
        ...
        <FinancialCard
          ...
          value={`ETB ${Math.round(displayExpenses).toLocaleString()}`}
          additionalContent={
            <div
        ...
        <FinancialCard
          ...
          value={`ETB ${Math.round(displaySavings).toLocaleString()}`}
          additionalContent={
              ...
              >
                {financialOverviewData.map((entry, index) => (
                  <Cell
                    key={`cell-${index}`}
                    fill={
                      MODERN_CHART_COLORS[index % MODERN_CHART_COLORS.length]
                    }
                    stroke="#fff"
                    strokeWidth={2}
                  />
                ...
                >
                  ...
                  <div className={dashboardStyles.transactionAmount}>
                    <p className={dashboardStyles.incomeAmount}>
                      +ETB {Math.abs(transaction.amount).toLocaleString()}
                    </p>
